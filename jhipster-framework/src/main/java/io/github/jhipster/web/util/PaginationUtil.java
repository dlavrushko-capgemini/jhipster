...
public final class PaginationUtil {
    ...
    private static String prepareLink(UriComponentsBuilder uriBuilder, int pageNumber, int pageSize, String relType) {
        return String.format(HEADER_LINK_FORMAT, preparePageUri(uriBuilder, pageNumber, pageSize), relType);
    }

    private static String preparePageUri(UriComponentsBuilder uriBuilder, int pageNumber, int pageSize) {
        return uriBuilder.replaceQueryParam("page", String.valueOf(pageNumber))
                         .replaceQueryParam("size", String.valueOf(pageSize))
                         .toUriString()
                         .replace(",", "%2C")
                         .replace(";", "%3B");
    }
