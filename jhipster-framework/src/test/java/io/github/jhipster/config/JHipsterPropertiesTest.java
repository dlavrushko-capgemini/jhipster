...
public class JHipsterPropertiesTest {
    ...
    public void testApiDocsProtocols() {
        ...
        if (def != null) {
            val.addAll(List.of(def));
            assertThat(obj.getProtocols()).containsExactlyElementsOf(List.copyOf(val));
        } else {
    ...
    @Test
        public void testCacheHazelcastManagementCenterUpdateInterval() {
            var obj = properties.getCache().getHazelcast().getManagementCenter();
    
            var val = JHipsterDefaults.Cache.Hazelcast.ManagementCenter.updateInterval;
    ...
    @Test
        public void testCacheInfinispanDistributedInstanceCount() {
            var obj = properties.getCache().getInfinispan().getDistributed();
            var val = JHipsterDefaults.Cache.Infinispan.Distributed.instanceCount;
    ...
    @Test
        public void testCacheInfinispanLocalMaxEntries() {
            var obj = properties.getCache().getInfinispan().getLocal();
    ...
    @Test
        public void testCacheInfinispanReplicatedMaxEntries() {
            var obj = properties.getCache().getInfinispan().getReplicated();
    ...
    @Test
        public void testGatewayRateLimitingEnabled() {
            var obj = properties.getGateway().getRateLimiting();
            var val = JHipsterDefaults.Gateway.RateLimiting.enabled;
            assertThat(obj.isEnabled()).isEqualTo(val);
            val = !val;
            obj.setEnabled(val);
            assertThat(obj.isEnabled()).isEqualTo(val);
    ...
    @Test
        public void testSecurityAuthenticationJwtBase64Secret() {
            var obj = properties.getSecurity().getAuthentication().getJwt();
            var val = JHipsterDefaults.Security.Authentication.Jwt.base64Secret;
    ...
    @Test
        public void testSecurityAuthenticationJwtSecret() {
            var obj = properties.getSecurity().getAuthentication().getJwt();
            var val = JHipsterDefaults.Security.Authentication.Jwt.secret;
            assertThat(obj.getSecret()).isEqualTo(val);
            val = "1" + val;
            obj.setSecret(val);
            assertThat(obj.getSecret()).isEqualTo(val);
    ...
    @Test
        public void testSecurityOauth2Audience() {
            ...
            obj.setAudience(List.of("default", "account"));
